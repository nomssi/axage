...
CLASS ltcl_engine IMPLEMENTATION.
  ...
  METHOD map_simple.
    ...
    DATA(room_left) = NEW room( name = 'LEFT' descr = 'Left room' ).
    DATA(room_right) = NEW room( name = 'RIGHT' descr = 'Right room' ).
  ...
  METHOD take_and_drop.
    ...
    DATA(room_right) = NEW room( name = 'RIGHT' descr = 'Right room' ).
    room_left->set_exits( e = room_right ).
    DATA(bottle) = NEW thing( name = 'BOTTLE' descr = 'an empty bottle' ).
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = VALUE thinglist=>_things( (  bottle ) ) ).
